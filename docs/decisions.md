...
- **T-025 `eslint-plugin-boundaries` works on ESLint 10 flat config, but needed two fixes to actually enforce anything.** The draft report flagged this as untried, with `no-restricted-imports` as the documented fallback. Tried directly, in a genuinely verified way (not just "no config error"): first attempt logged only deprecation warnings and silently let a deliberately-wrong deep import through. Root-caused by instrumenting the installed rule with temporary `console.error` (reverted after) rather than guessing further from summarized docs: (1) `target: ['core', 'service']` (bare string array) is a "legacy selector" the plugin's compat shim doesn't actually convert correctly — needed the object form `target: { element: { type: ['core', 'service'] } }`; (2) more importantly, the plugin resolves import specifiers via `eslint-import-resolver-node`, which only tries `.js`/`.json`/`.node` by default, so every extension-less `.ts` import failed to resolve (`dependency.to.element.types` came back `null`) and the rule silently skipped every real file in this repo. Fixed with `settings['import/resolver'] = { node: { extensions: ['.js', '.jsx', '.ts', '.tsx'] } }`. Verified for real afterward: a deep import bypassing `index.ts` now produces `boundaries/entry-point`'s error; reverted the temporary fixture and the rule-file edit once confirmed. The deprecation warnings for `entry-point`/`rules` (in favour of `dependencies`/`policies`) remain and are still tracked for a future migration. The simpler "core never imports a service", "routes never import a repo", and "web never imports worker code" rules use plain `no-restricted-imports` instead of the boundaries plugin, since their schema is unambiguous and well-known.
