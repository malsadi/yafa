...
- **T-040 Term currency (D-019) implemented as one SQL predicate, `end_date` compared against "today" in Europe/London, date granularity only.** D-019 defines currency purely by `end_date` versus now, with no separate status column. The query filters `end_date IS NULL OR end_date > :today`, where `:today` is a `YYYY-MM-DD` string (matching `start_date`/`end_date`'s own granularity — these are calendar dates an officer enters, not timestamps) computed via `Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/London' })`. Two things D-019 doesn't cover, decided here as technical/internal choices rather than asked about, and flagged for the owner batch rather than silently assumed correct: (1) **which civil timezone "today" uses** — Europe/London was chosen because brief section 9.1 already fixes it as the display timezone for the whole portal and every officer is UK-based; UTC was rejected because it can put "today" a day ahead of the UK's actual calendar date for part of the evening; (2) **a term with a future `start_date`** — D-019's wording defines currency only by `end_date`, so a term is treated as current from the moment it's created regardless of `start_date`, matching the letter of D-019 rather than inventing an "upcoming" status the brief's two-value `Current → Past officer` (section 14) doesn't name. Neither case can actually occur yet (Phase 0 seeds no terms), so this doesn't block anything; both are added to the next owner batch as narrow confirmations, not reopened as blocking questions.
- **T-041 Permission scope resolution (`own unit` / `all units` / `national content`, brief section 7.2) implemented as one pure function, `resolveScope()`, with its own unit tests.** Brief section 7.2 names the three scopes but does not define their comparison rules; section 7.1 says the General Council "owns national content," which is a real `units` row (`type = 'national'`), so `national content` is read as: the grant applies when the requested `unitId` equals that row's id (content owned by the national unit is unit-scoped data like any other, just owned by the national unit specifically) — not the settings-only `NATIONAL_SCOPE` sentinel (T-033), which is explicitly "not a real unit id" and is a different concept entirely. `own unit` requires the requested `unitId` to equal the specific term's own `unitId` (not any of the person's other current terms — see the cross-unit leak this closes, next entry). `all units` matches any requested `unitId` unconditionally, including the national unit's, since "all" is read as inclusive of every unit row without exception. This is a genuine specification gap, not a confident reading: it doesn't block building, because the permissions matrix (the data that actually grants `national content`-scoped capabilities to any role) stays empty until Phase 1 (15 A3) and nothing acts on this logic yet. Flagged for the owner batch: see O-011 in "Open".
...
O-002, O-005 (build-order half), O-006, O-008 (visibility half) were answered on 2026-09-22 — see D-017, D-019, D-020, D-021. O-003, O-004, O-007 (a)/(b) and O-009 were answered for real on 2026-09-22, this time — see D-023 to D-026. O-007's digits part, and two new narrow items found while acting on those answers, stay open below.
...
| O-010 | **Are system administrators exempt from the O-003/D-024 privacy-notice gate?** D-024 says no officer can be invited or sign in beyond "access not active" until a notice exists, and the notice is set by an administrator — but the first administrators arrive via Phase 1 seed, not an in-portal invite, and D-024's answer doesn't say whether *their* sign-in is also gated behind a notice that doesn't exist yet | First-sign-in screen and the administrator's own sign-in path (not built this session; doesn't block this session's work) |
| O-011 | **Permission scope semantics: does `all units` include the national unit, and what may `national content` scope let a *branch* officer do (if anything)?** T-041 implements a literal reading (`all units` = every unit row unconditionally; `national content` = the requested unit id equals the General Council's row) with its own tests, but the brief only names the three scopes, it doesn't define their comparison rules, and the permissions matrix that would actually exercise this stays empty until Phase 1 (15 A3) | Doesn't block Phase 0; confirm before the matrix is filled in during Phase 1 review |
