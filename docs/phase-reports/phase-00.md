...
| Clerk: middleware, signed webhook, invite-only, access-not-active page | **Middleware done this session (T-064): `requireSignedIn`/`requireActiveAccess`, backed by `verifyClerkSessionToken()` and `resolveSessionState()`.** Signed webhook, invite-only flow, and the actual "access not active" *page* (a frontend concern) are not built — webhook is next (its own commit), the rest is Phase 1+/frontend shell. |
...
| Documentation | `docs/decisions.md` through **T-064** and **O-017** (this session). This file, brought current. |
...
- **This report had fallen six commits behind** (T-051 to T-056 were built and committed across earlier sessions but never written up here). Brought current this session, through T-064.
...
4. Clerk middleware, webhook, `/api/me`:
   - ~~Session middleware (`requireSignedIn`/`requireActiveAccess`), the privacy-notice gate (`core/privacy-notice`), `/api/me`, and the permission sweep's behavioural half~~ (T-064) — done, this session, its own commit.
   - **The signed webhook (`user.created`/`user.updated`/`user.deleted` → link/sync/unlink by email, `verifyWebhook`) — not built yet.** This is the next piece, deliberately its own commit (advisor's guidance: two commits for this step, not one). Read brief section 6.2 again before starting: "a deleted Clerk user leaves the person record in place, unlinked" — never delete the person row. What to do when an email matches no `people` row at all is genuinely not stated in the brief; the safe default (do nothing, no error) is a reasonable technical call, but if it feels consequential once actually written, ask rather than assume. Match email case-insensitively (T-049). The real endpoint needs a new `.dev.vars.example` secret name and a Clerk dashboard endpoint on a public URL (needs `preview` to exist) — both are account actions to batch for the owner, not do silently.
...
4. Check `npx wrangler whoami` first if picking up `core/pdf` — if it still fails, that's still the blocker, not a new problem to debug. Otherwise go to step 4's webhook bullet above; nothing else is mid-flight.
