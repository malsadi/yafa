...
| Sub-point | State |
|---|---|
| Repository | Committed to `git@github.com:malsadi/yafa.git` (D-017); four commits on `main` locally, the latest of this session not yet pushed. |
...
| Documentation | `docs/decisions.md`: D-017 to D-022 (earlier), **D-023 to D-026 (this session: O-009/O-003/O-004/O-007 answered for real), T-038 to T-050 (this session's identity-tables/can()/scope work and three small fixes)**. This file. |
...
- `npm test` — **13 test files, 60 tests, all passing** (was 8 files / 36 tests earlier this session).
...
| # | Question | Blocks |
|---|---|---|
| O-010 | Are system administrators exempt from the D-024 privacy-notice gate, and from needing a current `terms` row to reach a working screen — since the first ones arrive via Phase 1 seed, not an in-portal invite? | Phase 1's first-sign-in screen and the administrator's own sign-in path |
| O-011 | Permission scope semantics: does `all units` include the national unit, and what may `national content` scope let a *branch* officer do, if anything? This session implements a literal, tested reading (T-041/T-050) since the brief names the three scopes but doesn't define their comparison rules | Phase 1, before the permissions matrix (15 A3) is filled in |
| O-012 | Does a term with a future `start_date` grant its powers immediately, or only from that date? This session's literal reading of D-019 (currency defined only by `end_date`) grants immediately, which fails open, not closed, if that's wrong | Phase 1, before a register officer can set a future start date |
...
3. Clerk middleware, webhook, `/api/me`. The middleware also applies D-024's privacy-notice gate (and O-010, once answered). Then write the sweep's behavioural half for real (fixture routes that call `can()`, signed in as officers of two different branches, over HTTP).
