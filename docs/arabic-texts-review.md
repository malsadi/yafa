...
| `accounts.sections.label` | Treasury sections | <span dir="rtl">أقسام الخزينة</span> |
| `accounts.sections.accounts` | Accounts | <span dir="rtl">الحسابات</span> |
| `accounts.sections.approvals` | Awaiting approval | <span dir="rtl">بانتظار الاعتماد</span> |
| `accounts.sections.years` | Financial years | <span dir="rtl">السنوات المالية</span> |
| `accounts.heading` | Accounts | <span dir="rtl">الحسابات</span> |
| `accounts.none` | No accounts yet. | <span dir="rtl">لا توجد حسابات بعد.</span> |
| `accounts.unitTotal` | Total of open accounts: {total} | <span dir="rtl">مجموع الحسابات المفتوحة: {total}</span> |
| `accounts.kinds.bank` | Bank | <span dir="rtl">بنك</span> |
| `accounts.kinds.cash` | Cash | <span dir="rtl">نقد</span> |
| `accounts.kinds.event` | Event | <span dir="rtl">فعالية</span> |
| `accounts.statuses.Open` | Open | <span dir="rtl">مفتوح</span> |
| `accounts.statuses.Closed` | Closed | <span dir="rtl">مغلق</span> |
| `accounts.awaiting` | {count} awaiting approval | <span dir="rtl">{count} بانتظار الاعتماد</span> |
| `accounts.belowZero` | Below zero | <span dir="rtl">دون الصفر</span> |
| `accounts.open` | Open a branch account | <span dir="rtl">فتح حساب للفرع</span> |
| `accounts.name` | Name | <span dir="rtl">الاسم</span> |
| `accounts.type` | Bank or cash | <span dir="rtl">بنك أو نقد</span> |
| `accounts.openingBalance` | Opening balance (£, may be negative) | <span dir="rtl">الرصيد الافتتاحي (£، قد يكون سالبًا)</span> |
| `accounts.openingDate` | Opened on | <span dir="rtl">تاريخ الفتح</span> |
| `accounts.save` | Open account | <span dir="rtl">فتح الحساب</span> |
| `accounts.cancel` | Cancel | <span dir="rtl">إلغاء</span> |
| `accounts.close` | Close account | <span dir="rtl">إغلاق الحساب</span> |
...
| `entries.record.credit` | Record a credit | <span dir="rtl">تسجيل إيراد</span> |
| `entries.record.debit` | Record a debit | <span dir="rtl">تسجيل مصروف</span> |
| `entries.record.transfer` | Record a transfer | <span dir="rtl">تسجيل تحويل</span> |
| `entries.types.opening-balance` | Opening balance | <span dir="rtl">رصيد افتتاحي</span> |
| `entries.types.credit` | Credit | <span dir="rtl">إيراد</span> |
| `entries.types.debit` | Debit | <span dir="rtl">مصروف</span> |
| `entries.types.transfer` | Transfer | <span dir="rtl">تحويل</span> |
| `entries.transferTo` | Transfer to {account} | <span dir="rtl">تحويل إلى {account}</span> |
| `entries.transferFrom` | Transfer from {account} | <span dir="rtl">تحويل من {account}</span> |
| `entries.approvalStatuses.Awaiting approval` | Awaiting approval | <span dir="rtl">بانتظار الاعتماد</span> |
| `entries.approvalStatuses.Approved` | Approved | <span dir="rtl">معتمد</span> |
| `entries.approvalStatuses.Declined` | Declined | <span dir="rtl">مرفوض</span> |
| `entries.account` | Account | <span dir="rtl">الحساب</span> |
| `entries.toAccount` | To account | <span dir="rtl">إلى الحساب</span> |
| `entries.amount` | Amount (£) | <span dir="rtl">المبلغ (£)</span> |
| `entries.date` | Date | <span dir="rtl">التاريخ</span> |
| `entries.source` | Source | <span dir="rtl">المصدر</span> |
| `entries.paidTo` | Paid to | <span dir="rtl">المدفوع له</span> |
| `entries.description` | Description | <span dir="rtl">الوصف</span> |
| `entries.budgetLine` | Budget line (optional) | <span dir="rtl">بند الميزانية (اختياري)</span> |
| `entries.receipts` | Receipt photos | <span dir="rtl">صور الإيصالات</span> |
| `entries.receiptsHint` | Take a photo with your camera, or choose one. | <span dir="rtl">التقط صورة بالكاميرا، أو اختر صورة.</span> |
| `entries.takePhoto` | Take photo | <span dir="rtl">التقاط صورة</span> |
| `entries.choosePhoto` | Choose photo | <span dir="rtl">اختيار صورة</span> |
| `entries.photosChosen` | {count} photo(s) ready | <span dir="rtl">{count} صورة جاهزة</span> |
| `entries.save` | Save | <span dir="rtl">حفظ</span> |
| `entries.done` | Done | <span dir="rtl">تم</span> |
| `entries.cancel` | Cancel | <span dir="rtl">إلغاء</span> |
| `entries.amountInvalid` | Write the amount in pounds, such as 150 or 20.50. | <span dir="rtl">اكتب المبلغ بالجنيه، مثل 150 أو 20.50.</span> |
| `entries.willGoBelowZero` | This takes {account} below zero. It will be saved, with a warning. | <span dir="rtl">هذا ينزل برصيد {account} دون الصفر. سيُحفظ مع تنبيه.</span> |
| `entries.enteredBy` | Entered by {name} on {date} | <span dir="rtl">سجّله {name} في {date}</span> |
| `entries.decidedBy` | {status} by {name} on {date} | <span dir="rtl">{status} بواسطة {name} في {date}</span> |
| `entries.declineReason` | Reason: {reason} | <span dir="rtl">السبب: {reason}</span> |
| `entries.reversalOf` | Reverses an earlier entry | <span dir="rtl">يعكس قيدًا سابقًا</span> |
| `entries.reversedBy` | Reversed by a later entry | <span dir="rtl">عُكس بقيد لاحق</span> |
| `entries.noReceipt` | No receipt | <span dir="rtl">لا إيصال</span> |
| `entries.addReceipt` | Add receipt | <span dir="rtl">إضافة إيصال</span> |
| `entries.download` | Receipt {number} | <span dir="rtl">الإيصال {number}</span> |
| `entries.correct` | Correct | <span dir="rtl">تصحيح</span> |
| `entries.correctExplanation` | This records a reversing entry, dated today, that undoes this one. Then record the right entry as a new one. | <span dir="rtl">يسجّل هذا قيدًا عكسيًا بتاريخ اليوم يلغي هذا القيد. ثم سجّل القيد الصحيح قيدًا جديدًا.</span> |
| `entries.correctNote` | Why (optional) | <span dir="rtl">السبب (اختياري)</span> |
| `entries.confirmCorrect` | Record reversing entry | <span dir="rtl">تسجيل القيد العكسي</span> |
| `entries.period.from` | From | <span dir="rtl">من</span> |
| `entries.period.to` | To | <span dir="rtl">إلى</span> |
| `entries.period.show` | Show | <span dir="rtl">عرض</span> |
| `entries.warnings.below-zero` | {account} is now below zero: {balance}. | <span dir="rtl">رصيد {account} الآن دون الصفر: {balance}.</span> |
| `entries.warnings.no-receipt` | Saved without a receipt. You can add one later. | <span dir="rtl">حُفظ دون إيصال. يمكنك إضافته لاحقًا.</span> |
| `entries.saved.Not needed` | Saved. | <span dir="rtl">تم الحفظ.</span> |
| `entries.saved.Awaiting approval` | Saved. It is above the threshold, so it waits for a second officer to approve it. | <span dir="rtl">تم الحفظ. هو فوق حد الاعتماد، لذا ينتظر اعتماد مسؤول آخر.</span> |
| `approvals.heading` | Awaiting approval | <span dir="rtl">بانتظار الاعتماد</span> |
| `approvals.explanation` | Debits and transfers above the threshold wait here until a second officer approves or declines them. | <span dir="rtl">تنتظر المصروفات والتحويلات التي تتجاوز الحد هنا حتى يعتمدها مسؤول آخر أو يرفضها.</span> |
| `approvals.none` | Nothing is awaiting approval. | <span dir="rtl">لا شيء بانتظار الاعتماد.</span> |
| `approvals.yours` | You entered this, so another officer decides it. | <span dir="rtl">أنت سجّلت هذا، لذا يقرره مسؤول آخر.</span> |
| `approvals.approve` | Approve | <span dir="rtl">اعتماد</span> |
| `approvals.decline` | Decline | <span dir="rtl">رفض</span> |
| `approvals.reason` | Reason for declining | <span dir="rtl">سبب الرفض</span> |
| `approvals.confirmDecline` | Decline | <span dir="rtl">رفض</span> |
| `statements.heading` | Statement | <span dir="rtl">كشف الحساب</span> |
| `statements.from` | From | <span dir="rtl">من</span> |
| `statements.to` | To | <span dir="rtl">إلى</span> |
| `statements.view` | View | <span dir="rtl">عرض</span> |
| `statements.download` | Download PDF | <span dir="rtl">تنزيل PDF</span> |
| `statements.file` | File to the archive | <span dir="rtl">حفظ في الأرشيف</span> |
| `statements.filed` | Filed to the archive, in Finance. | <span dir="rtl">حُفظ في الأرشيف، ضمن المالية.</span> |
| `statements.opening` | Balance at the start: {balance} | <span dir="rtl">الرصيد في البداية: {balance}</span> |
| `statements.closing` | Balance at the end: {balance} | <span dir="rtl">الرصيد في النهاية: {balance}</span> |
| `statements.columns.date` | Date | <span dir="rtl">التاريخ</span> |
| `statements.columns.details` | Details | <span dir="rtl">التفاصيل</span> |
| `statements.columns.in` | In | <span dir="rtl">وارد</span> |
| `statements.columns.out` | Out | <span dir="rtl">صادر</span> |
| `statements.columns.balance` | Balance | <span dir="rtl">الرصيد</span> |
| `years.heading` | Financial years | <span dir="rtl">السنوات المالية</span> |
| `years.explanation` | Closing a year locks its entries and files a statement for every account to the archive. A year closes once it has ended, after the year before it, and with nothing awaiting approval. | <span dir="rtl">إقفال السنة يقفل قيودها ويحفظ كشفًا لكل حساب في الأرشيف. تُقفل السنة بعد انتهائها، وبعد السنة التي قبلها، ولا شيء فيها بانتظار الاعتماد.</span> |
| `years.period` | {start} to {end} | <span dir="rtl">من {start} إلى {end}</span> |
| `years.closed` | Closed by {name} on {date} | <span dir="rtl">أقفلها {name} في {date}</span> |
| `years.closedNoName` | Closed on {date} | <span dir="rtl">أُقفلت في {date}</span> |
| `years.open` | Open | <span dir="rtl">مفتوحة</span> |
| `years.notEnded` | Not ended yet | <span dir="rtl">لم تنتهِ بعد</span> |
| `years.awaiting` | {count} awaiting approval | <span dir="rtl">{count} بانتظار الاعتماد</span> |
| `years.close` | Close year | <span dir="rtl">إقفال السنة</span> |
| `years.confirm` | Close this year? Its entries will be locked for good. | <span dir="rtl">إقفال هذه السنة؟ ستُقفل قيودها نهائيًا.</span> |
| `refusals.permission.denied` | You may not do this. | <span dir="rtl">لا يحق لك القيام بذلك.</span> |
| `refusals.branches.inactive` | This branch is inactive, so its Treasury is read-only. | <span dir="rtl">هذا الفرع غير نشط، لذا فخزينته للاطلاع فقط.</span> |
...
| `refusals.pdf.not-available` | PDFs are not available here. They work on the preview site. | <span dir="rtl">ملفات PDF غير متاحة هنا. تعمل في موقع المعاينة.</span> |
...
| `refusals.treasury.closed-year` | That date is in a closed financial year, whose entries are locked. | <span dir="rtl">هذا التاريخ في سنة مالية مُقفلة، قيودها مقفلة.</span> |
| `refusals.treasury.receipt-required` | A receipt photo is required for this. | <span dir="rtl">صورة الإيصال مطلوبة لهذا.</span> |
| `refusals.treasury.budget-line-not-of-account` | Choose a budget line of this event account. | <span dir="rtl">اختر بند ميزانية من هذا الحساب.</span> |
| `refusals.treasury.entry-exists` | This entry has already been saved. | <span dir="rtl">حُفظ هذا القيد من قبل.</span> |
| `refusals.treasury.entry-not-found` | This entry does not exist. | <span dir="rtl">هذا القيد غير موجود.</span> |
| `refusals.treasury.receipt-not-found` | This receipt does not exist. | <span dir="rtl">هذا الإيصال غير موجود.</span> |
| `refusals.treasury.not-awaiting` | This is no longer awaiting approval. | <span dir="rtl">لم يعد هذا بانتظار الاعتماد.</span> |
| `refusals.treasury.self-approval` | You entered this, so a second officer must decide it. | <span dir="rtl">أنت سجّلت هذا، فيجب أن يقرره مسؤول آخر.</span> |
| `refusals.treasury.reason-required` | Give a reason for declining. | <span dir="rtl">اذكر سبب الرفض.</span> |
| `refusals.treasury.already-reversed` | This entry has already been reversed. | <span dir="rtl">عُكس هذا القيد من قبل.</span> |
| `refusals.treasury.reversal-not-reversed` | A reversing entry cannot itself be reversed. | <span dir="rtl">لا يمكن عكس قيد عكسي.</span> |
| `refusals.treasury.not-counted` | Only an entry that counts can be reversed. | <span dir="rtl">لا يُعكس إلا قيد محتسب.</span> |
| `refusals.treasury.statement-already-filed` | This statement has already been filed. | <span dir="rtl">حُفظ هذا الكشف من قبل.</span> |
| `refusals.treasury.not-a-year-start` | That is not the start of a financial year. | <span dir="rtl">هذا ليس بداية سنة مالية.</span> |
| `refusals.treasury.year-closed` | This year is already closed. | <span dir="rtl">هذه السنة مُقفلة بالفعل.</span> |
| `refusals.treasury.year-not-ended` | This year has not ended yet. | <span dir="rtl">لم تنتهِ هذه السنة بعد.</span> |
| `refusals.treasury.earlier-year-open` | Close the earlier financial year first. | <span dir="rtl">أقفل السنة المالية السابقة أولًا.</span> |
| `refusals.treasury.awaiting-in-year` | {awaiting} dated in this year await approval. Decide them first. | <span dir="rtl">{awaiting} مؤرخة في هذه السنة بانتظار الاعتماد. قرّرها أولًا.</span> |
| `refusals.files.type-not-allowed` | This file type is not allowed for receipt photos. | <span dir="rtl">نوع هذا الملف غير مسموح به لصور الإيصالات.</span> |
| `refusals.files.too-large` | This photo is larger than the limit for receipt photos. | <span dir="rtl">حجم هذه الصورة أكبر من الحد المسموح به لصور الإيصالات.</span> |
...
| `sections.templates` | Templates | <span dir="rtl">النماذج</span> |
| `sections.guides` | Guides | <span dir="rtl">الأدلة</span> |
| `sections.venues` | Venues | <span dir="rtl">القاعات</span> |
| `sections.equipment` | Equipment | <span dir="rtl">المعدات</span> |
...
| `resources.headings.template` | Templates | <span dir="rtl">النماذج</span> |
| `resources.headings.guide` | Guides | <span dir="rtl">الأدلة</span> |
| `resources.explanation.template` | Forms, sign-in sheets, flyer and poster designs, and other ready-to-use files. | <span dir="rtl">الاستمارات وكشوف الحضور وتصاميم المنشورات والملصقات، وغيرها من الملفات الجاهزة للاستخدام.</span> |
| `resources.explanation.guide` | Role handbooks, procedures and how-to guides for officers. | <span dir="rtl">أدلة المناصب والإجراءات وأدلة الإرشاد للمسؤولين.</span> |
| `resources.none` | Nothing here yet. | <span dir="rtl">لا يوجد شيء هنا بعد.</span> |
| `resources.add.template` | Add a template | <span dir="rtl">إضافة نموذج</span> |
| `resources.add.guide` | Add a guide | <span dir="rtl">إضافة دليل</span> |
| `resources.file` | File | <span dir="rtl">الملف</span> |
| `resources.title` | Title | <span dir="rtl">العنوان</span> |
| `resources.description` | Description (optional) | <span dir="rtl">الوصف (اختياري)</span> |
| `resources.language` | Language of the file | <span dir="rtl">لغة الملف</span> |
| `resources.languages.en` | English | <span dir="rtl">الإنجليزية</span> |
| `resources.languages.ar` | Arabic | <span dir="rtl">العربية</span> |
| `resources.save` | Save | <span dir="rtl">حفظ</span> |
| `resources.cancel` | Cancel | <span dir="rtl">إلغاء</span> |
| `resources.edit` | Change details | <span dir="rtl">تعديل التفاصيل</span> |
| `resources.replace` | Replace file | <span dir="rtl">استبدال الملف</span> |
| `resources.replaceExplanation` | The new file is shown from now on; the old one is kept. | <span dir="rtl">يظهر الملف الجديد من الآن؛ ويُحتفظ بالقديم.</span> |
| `resources.download` | Download | <span dir="rtl">تنزيل</span> |
| `resources.retire` | Retire | <span dir="rtl">إيقاف</span> |
| `resources.restore` | Bring back | <span dir="rtl">إعادة</span> |
| `resources.national` | General Council | <span dir="rtl">المجلس العام</span> |
| `resources.retired` | Retired | <span dir="rtl">موقوف</span> |
| `resources.refusals.permission.denied` | You may not do this. | <span dir="rtl">لا يحق لك القيام بذلك.</span> |
| `resources.refusals.resources-library.stale` | Someone else changed this. Reload the page to see their change. | <span dir="rtl">عدّل شخص آخر هذا. أعد تحميل الصفحة لترى تعديله.</span> |
| `resources.refusals.resources-library.resource-not-found` | This no longer exists. | <span dir="rtl">هذا لم يعد موجودًا.</span> |
| `resources.refusals.resources-library.already-retired` | This is already retired. | <span dir="rtl">هذا موقوف بالفعل.</span> |
| `resources.refusals.resources-library.not-retired` | This is not retired. | <span dir="rtl">هذا غير موقوف.</span> |
| `resources.refusals.branches.inactive` | This branch is inactive, so its library is read-only. | <span dir="rtl">هذا الفرع غير نشط، لذا فمكتبته للاطلاع فقط.</span> |
| `resources.refusals.files.type-not-allowed` | This file type is not allowed for documents. | <span dir="rtl">نوع هذا الملف غير مسموح به للوثائق.</span> |
| `resources.refusals.files.too-large` | This file is larger than the limit for documents. | <span dir="rtl">حجم هذا الملف أكبر من الحد المسموح به للوثائق.</span> |
| `resources.refusals.files.storage-not-configured` | File storage is not set up yet. | <span dir="rtl">لم يُجهَّز تخزين الملفات بعد.</span> |
| `resources.refusals.files.upload-failed` | The upload did not finish. Try again. | <span dir="rtl">لم يكتمل الرفع. حاول مرة أخرى.</span> |
| `resources.refusals.files.not-uploaded` | The upload did not finish. Try again. | <span dir="rtl">لم يكتمل الرفع. حاول مرة أخرى.</span> |
| `resources.refusals.setting.not-configured` | This has not been set up yet. Please ask your administrator. | <span dir="rtl">لم يتم إعداد هذا بعد. يرجى التواصل مع المسؤول.</span> |
| `venues.heading` | Venues | <span dir="rtl">القاعات</span> |
| `venues.none` | No venues yet. | <span dir="rtl">لا توجد قاعات بعد.</span> |
| `venues.add` | Add a venue | <span dir="rtl">إضافة قاعة</span> |
| `venues.name` | Name | <span dir="rtl">الاسم</span> |
| `venues.address` | Address | <span dir="rtl">العنوان</span> |
| `venues.capacity` | Capacity | <span dir="rtl">السعة</span> |
| `venues.facilities` | Facilities | <span dir="rtl">المرافق</span> |
| `venues.contactName` | Contact person | <span dir="rtl">الشخص المسؤول</span> |
| `venues.contactPhone` | Contact phone | <span dir="rtl">هاتف المسؤول</span> |
| `venues.contactEmail` | Contact email | <span dir="rtl">بريد المسؤول الإلكتروني</span> |
| `venues.typicalCost` | Typical cost (£) | <span dir="rtl">التكلفة المعتادة (£)</span> |
| `venues.typicalCostNote` | What the cost covers | <span dir="rtl">ما تشمله التكلفة</span> |
| `venues.optional` | {label} (optional) | <span dir="rtl">{label} (اختياري)</span> |
| `venues.costInvalid` | Write the cost in pounds, such as 150 or 150.50. | <span dir="rtl">اكتب التكلفة بالجنيه، مثل 150 أو 150.50.</span> |
| `venues.people` | {count} people | <span dir="rtl">{count} شخصًا</span> |
| `venues.save` | Save | <span dir="rtl">حفظ</span> |
| `venues.cancel` | Cancel | <span dir="rtl">إلغاء</span> |
| `venues.edit` | Change details | <span dir="rtl">تعديل التفاصيل</span> |
| `venues.retire` | Retire | <span dir="rtl">إيقاف</span> |
| `venues.restore` | Bring back | <span dir="rtl">إعادة</span> |
| `venues.national` | General Council | <span dir="rtl">المجلس العام</span> |
| `venues.retired` | Retired | <span dir="rtl">موقوفة</span> |
| `venues.notes` | Notes from past use | <span dir="rtl">ملاحظات من الاستخدام السابق</span> |
| `venues.noNotes` | No notes yet. | <span dir="rtl">لا توجد ملاحظات بعد.</span> |
| `venues.noteBy` | {name}, {date} | <span dir="rtl">{name}، {date}</span> |
| `venues.noteOn` | {date} | <span dir="rtl">{date}</span> |
...
| `venues.refusals.resources-library.venue-note-not-found` | This note is no longer showing. | <span dir="rtl">هذه الملاحظة لم تعد ظاهرة.</span> |
| `venues.refusals.resources-library.already-retired` | This venue is already retired. | <span dir="rtl">هذه القاعة موقوفة بالفعل.</span> |
| `venues.refusals.resources-library.not-retired` | This venue is not retired. | <span dir="rtl">هذه القاعة غير موقوفة.</span> |
| `venues.refusals.branches.inactive` | This branch is inactive, so its library is read-only. | <span dir="rtl">هذا الفرع غير نشط، لذا فمكتبته للاطلاع فقط.</span> |
| `equipment.heading` | Equipment | <span dir="rtl">المعدات</span> |
| `equipment.none` | No equipment yet. | <span dir="rtl">لا توجد معدات بعد.</span> |
| `equipment.add` | Add equipment | <span dir="rtl">إضافة معدات</span> |
| `equipment.item` | Item | <span dir="rtl">الصنف</span> |
...
| `equipment.edit` | Change details | <span dir="rtl">تعديل التفاصيل</span> |
| `equipment.retire` | Retire | <span dir="rtl">إيقاف</span> |
| `equipment.restore` | Bring back | <span dir="rtl">إعادة</span> |
| `equipment.national` | General Council | <span dir="rtl">المجلس العام</span> |
| `equipment.retired` | Retired | <span dir="rtl">موقوف</span> |
| `equipment.onLoanNow` | Out on loan now | <span dir="rtl">المعار حاليًا</span> |
| `equipment.nothingOut` | Nothing is out on loan. | <span dir="rtl">لا شيء معار حاليًا.</span> |
| `equipment.loans` | Loans | <span dir="rtl">الإعارات</span> |
| `equipment.lend` | Lend | <span dir="rtl">إعارة</span> |
| `equipment.borrower` | Borrower | <span dir="rtl">المستعير</span> |
| `equipment.borrowedOn` | Date borrowed | <span dir="rtl">تاريخ الإعارة</span> |
| `equipment.dueBack` | Due back | <span dir="rtl">موعد الإرجاع</span> |
| `equipment.returnedOn` | Date it came back | <span dir="rtl">تاريخ الإرجاع الفعلي</span> |
| `equipment.loanSummary` | {quantity} × {item} to {borrower}, from {borrowed}, due back {due} | <span dir="rtl">{quantity} × {item} إلى {borrower}، من {borrowed}، موعد الإرجاع {due}</span> |
| `equipment.loanReturned` | Came back on {date} | <span dir="rtl">أُرجعت في {date}</span> |
| `equipment.correct` | Correct | <span dir="rtl">تصحيح</span> |
| `equipment.recordReturn` | Record return | <span dir="rtl">تسجيل الإرجاع</span> |
| `equipment.history` | History | <span dir="rtl">السجل</span> |
| `equipment.changes.lent` | Lent | <span dir="rtl">أُعيرت</span> |
| `equipment.changes.corrected` | Corrected | <span dir="rtl">صُحّحت</span> |
| `equipment.changes.returned` | Returned | <span dir="rtl">أُرجعت</span> |
| `equipment.historyEntry` | {change} by {name} on {date}: {quantity} to {borrower}, {borrowed} to {due} | <span dir="rtl">{change} بواسطة {name} في {date}: {quantity} إلى {borrower}، من {borrowed} إلى {due}</span> |
| `equipment.refusals.permission.denied` | You may not do this. | <span dir="rtl">لا يحق لك القيام بذلك.</span> |
| `equipment.refusals.resources-library.stale` | Someone else changed this. Reload the page to see their change. | <span dir="rtl">عدّل شخص آخر هذا. أعد تحميل الصفحة لترى تعديله.</span> |
| `equipment.refusals.resources-library.equipment-not-found` | This item no longer exists. | <span dir="rtl">هذا الصنف لم يعد موجودًا.</span> |
| `equipment.refusals.resources-library.loan-not-found` | This loan no longer exists. | <span dir="rtl">هذه الإعارة لم تعد موجودة.</span> |
| `equipment.refusals.resources-library.already-retired` | This item is already retired. | <span dir="rtl">هذا الصنف موقوف بالفعل.</span> |
| `equipment.refusals.resources-library.not-retired` | This item is not retired. | <span dir="rtl">هذا الصنف غير موقوف.</span> |
| `equipment.refusals.resources-library.condition-not-offered` | Choose a condition from the list. | <span dir="rtl">اختر حالة من القائمة.</span> |
| `equipment.refusals.resources-library.equipment-retired` | This item is retired, so it cannot be lent. | <span dir="rtl">هذا الصنف موقوف، فلا يمكن إعارته.</span> |
| `equipment.refusals.resources-library.over-lent` | Only {left} left to lend: {quantity} owned and {out} already out on loan. You asked for {requested}. | <span dir="rtl">المتبقي للإعارة {left} فقط: المملوك {quantity} والمعار حاليًا {out}. طلبت {requested}.</span> |
| `equipment.refusals.resources-library.below-loans` | The quantity cannot be {quantity}: {out} are out on loan now. | <span dir="rtl">لا يمكن أن تكون الكمية {quantity}: المعار حاليًا {out}.</span> |
| `equipment.refusals.resources-library.loan-returned` | This loan has been returned, so it can no longer change. | <span dir="rtl">أُرجعت هذه الإعارة، فلم يعد بالإمكان تغييرها.</span> |
| `equipment.refusals.resources-library.returned-before-borrowed` | It cannot come back before the day it was borrowed. | <span dir="rtl">لا يمكن أن يُرجع قبل يوم إعارته.</span> |
| `equipment.refusals.branches.inactive` | This branch is inactive, so its library is read-only. | <span dir="rtl">هذا الفرع غير نشط، لذا فمكتبته للاطلاع فقط.</span> |
| `equipment.refusals.request.invalid` | Check the details: it is due back on or after the day it was borrowed. | <span dir="rtl">راجع التفاصيل: موعد الإرجاع في يوم الإعارة أو بعده.</span> |
