...
| `administration-panel.service-switches.manage` | **Switch services on and off.** Turn services on or off portal-wide or per unit, with dependency checks; never deletes data (25 C2, 8.4). | all units | System administrators always (D-046); anyone else through the permissions matrix. |
| `administration-panel.notifications.manage` | **Set notification defaults.** The alert types switched on for new officers, and the iPhone install guide text in English and Arabic (25 C4). | all units | System administrators always (D-046); anyone else through the permissions matrix. |
| `administration-panel.texts.manage` | **Write the texts.** The privacy notice (each change a new version every officer reads again), the "access not active" message and the help text, in English and Arabic (25 C5). | all units | System administrators always (D-046); anyone else through the permissions matrix. |
| `administration-panel.branding.manage` | **Set the branding and letterhead.** The organisation name in English and Arabic, the main and accent colours, and the letterhead (25 C3). | all units | System administrators always (D-046); anyone else through the permissions matrix. |
